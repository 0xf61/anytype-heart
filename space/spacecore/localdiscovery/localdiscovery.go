...
// PeerDiscovered seam as mDNS, using our own l.ipv4/l.port as the dial-back
// hint (the remote also learns our address from the connection source during
// SpaceExchange, so this is best-effort). No-op when static-peers.json is
// absent, so the feature is opt-in and own-address.json is not written for
...
func (l *localDiscovery) injectStaticPeers(ctx context.Context) {
	...
	for _, p := range peers {
		if p.PeerId == "" || p.PeerId == l.peerId || len(p.Addresses) == 0 {
			continue
		}
		if l.notifier != nil {
			peer := DiscoveredPeer{PeerId: p.PeerId, Addrs: p.Addresses}
...
// writeOwnAddress writes own-address.json so the user can read this client's
// peerId + listen port + listen addresses to share with the remote peer's
// static-peers.json. Only written when static-peers.json exists (opt-in).
// Skipped when the content is unchanged to avoid disk churn.
//
// The listen addr is advertised as yamux:// to match addSchema, which dials
// local peers over yamux (TCP) only since GO-7424 — TCP fails fast (RST) on a
// dead peer, unlike QUIC's full handshake timeout.
func (l *localDiscovery) writeOwnAddress(own OwnAddresses) {
	...
	doc := map[string]any{
		...
		"addrs":       own.Addrs,
		"listenAddrs": listen,
		"note":        "share peerId + a reachable listenAddr with the remote peer's static-peers.json",
	}
